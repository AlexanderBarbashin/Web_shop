...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="username", required=True, type=str, description="Имя пользователя"
        ),
        OpenApiParameter(
            name="password", required=True, type=str, description="Пароль"
        ),
    ],
    responses={
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            avatar = Avatar.objects.create(
                src=avatar_file, alt="{user} аватар".format(user=user)
            )
