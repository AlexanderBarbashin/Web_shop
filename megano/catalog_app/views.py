...
@extend_schema(tags=['catalog'],
               responses={
                   200: OpenApiResponse(response=200, description='successful operation', examples=[
                       OpenApiExample(
                           name='',
                           value={
                           'id': 123,
...
class CategoryView(ListAPIView):
    ...
    main_categories = Category.objects.filter(main_category=None).prefetch_related('subcategories').\
    ...
    for category in main_categories:
        subcategories_products_sum = category.subcategories.all().prefetch_related('products').\
            annotate(subcats_prods_count=Count('products')).aggregate(subcats_prods_sum=Sum('subcats_prods_count'))
        if category.prods_count == 0 and subcategories_products_sum['subcats_prods_sum'] == 0:
