...
class Basket(models.Model):
    ...
    def __str__(self) -> str:
        ...
        return "Корзина пользователя {username}".format(username=username)
...
class ProductsInBasketCount(models.Model):
    ...
    count_in_basket = models.PositiveSmallIntegerField(
    ...
    def __str__(self) -> str:
        ...
        return "Количество товаров {product} в корзине пользователя {username}: {count}".format(
            product=self.product, username=username, count=self.count_in_basket
        )
...
class Order(models.Model):
    ...
    totalCost = models.DecimalField(
        decimal_places=2, max_digits=10, null=True, verbose_name="Итоговая стоимость"
    )
...
class DeliveryPrice(models.Model):
    ...
    free_delivery_point = models.DecimalField(
        decimal_places=2, max_digits=10, verbose_name="Порог бесплатной доставки"
    )
